...
{
    m_sock = open("/dev/pipe_opengles", O_RDWR);
    if (m_sock < 0) {
    		ALOGE("%s: Could not open device pipe: %s", __FUNCTION__, strerror(errno));
    		return -3;
    	}
    if (!valid()) return -1;
    return 0;
...
int QemuPipeStream::writeFully(const void *buf, size_t len)
{
    ...
    if (!valid()) return -1;
    if (!buf) {
        if (len>0) ERR("PipeStream::writeFully failed, buf=NULL, len %d", len);
        return 0;
    }
    ...
    while (res > 0) {
        ...
        if (stat == 0) { /* EOF */
            ERR("PipeStream::writeFully failed: premature EOF\n");
            retval = -1;
            break;
        }
        if (errno == EINTR) {
        ...
        retval =  stat;
        ERR("PipeStream::writeFully failed: %s\n", strerror(errno));
        break;
    }
    //DBG("<< PipeStream::writeFully %d\n", len );
    return retval;
}

const unsigned char *QemuPipeStream::readFully(void *buf, size_t len)
{
    //DBG(">> PipeStream::readFully %d\n", len);
    if (!valid()) return NULL;
    if (!buf) {
        if (len > 0) {
            ERR("PipeStream::readFully failed, buf=NULL, len %zu", len);
        }
    ...
    while (res > 0) {
        ...
        if (stat == 0) {
            ...
            if (errno == EINTR) {
                ...
                ERR("PipeStream::readFully failed (buf %p, len %zu"
...
{
    //DBG(">> PipeStream::read %d\n", *inout_len);
    if (!valid()) return NULL;
    if (!buf) {
      ERR("PipeStream::read failed, buf=NULL");
      return NULL;  // do not allow NULL buf in that implementation
    }
