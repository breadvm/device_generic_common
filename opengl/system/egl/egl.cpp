...
EGLBoolean eglChooseConfig(EGLDisplay dpy, const EGLint *attrib_list, EGLConfig *configs, EGLint config_size, EGLint *num_config)
{
    ...
    if (attrib_list) {
        ...
        while (attrib_p[0] != EGL_NONE) {
            ...
            //ALOGE("eglChooseConfig send attrrib %x value %d", attrib_p[0], attrib_p[1]);
...
for(index;index<*num_config;index++)
{
if (!configs) break;
	s_display.getConfigAttrib(configs[index],EGL_RED_SIZE,&redSize);
	s_display.getConfigAttrib(configs[index],EGL_GREEN_SIZE,&greenSize);
	s_display.getConfigAttrib(configs[index],EGL_BLUE_SIZE,&blueSize);
	s_display.getConfigAttrib(configs[index],EGL_ALPHA_SIZE,&alphaSize);
	s_display.getConfigAttrib(configs[index],EGL_DEPTH_SIZE,&depthSize);
	s_display.getConfigAttrib(configs[index],EGL_STENCIL_SIZE,&stencilSize);
ALOGE("config index= %d:red= %d, green= %d, blue= %d,alpha= %d,depth= %d, stencil= %d\n", index, redSize, greenSize, blueSize, alphaSize, depthSize, stencilSize);
