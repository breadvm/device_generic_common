...
bool eglDisplay::initialize(EGLClient_eglInterface *eglIface)
{
    ...
    if (!m_initialized) {
        ...
        //
#if __LP64__
        m_gles_iface = loadGLESClientAPI("/system/lib64/egl/libGLESv1_CM_xxbos.so",
                                         eglIface,
                                         &s_gles_lib);
#else
        m_gles_iface = loadGLESClientAPI("/system/lib/egl/libGLESv1_CM_xxbos.so",
                                         eglIface,
                                         &s_gles_lib);
#endif
        if (!m_gles_iface) {
...
#ifdef WITH_GLES2
#if __LP64__
        m_gles2_iface = loadGLESClientAPI("/system/lib64/egl/libGLESv2_xxbos.so",
                                          eglIface,
                                          &s_gles2_lib);
#else
        m_gles2_iface = loadGLESClientAPI("/system/lib/egl/libGLESv2_xxbos.so",
                                          eglIface,
                                          &s_gles2_lib);
#endif
...
static char *buildExtensionString()
{
    ...
    if (n > 0) {
        char *finalEGLExts;
        asprintf(&finalEGLExts,"%s%s", systemStaticEGLExtensions, hostExt);
        free((char*)hostExt);
        return finalEGLExts;
    }
    else {
